...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!flag)
    {
        ...
        if (selectCount>self.maximumImagesCount) {
        [dic setObject:@"0" forKey:@"flag"];
        NSString *title = [NSString stringWithFormat:@"您最多可以选择%ld张图片",(long)self.maximumImagesCount];
        NSString *message = [NSString stringWithFormat:NSLocalizedString(@"您最多一次 %d 图片", nil), self.maximumImagesCount];
    ...
    if (selectCount > 0)
    {
        ...
        countLabel.text = [NSString stringWithFormat:@"%ld", (long)selectCount];
    } else {
        endBtn.enabled = NO;
        endBtn.backgroundColor = [UIColor clearColor];
        countImageView.hidden = YES;
